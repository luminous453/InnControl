...
# Получение объекта пользователя по данным из JWT
async def get_current_user(token: str = Depends(oauth2_scheme)):
    ...
    # Хардкодированная проверка для админа (не используем базу данных)
    if username == "admin":
        user = schemas.User(id=1, username="admin", email="admin@inncontrol.com", is_active=True, is_admin=True)
        return user
    raise credentials_exception
